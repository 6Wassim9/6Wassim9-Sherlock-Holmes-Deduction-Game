...
int main(int argc, char *argv[])
{
	...
	else if (fsmServer==1)
	{
		switch (buffer[0])
		{
			    ...
			    {
                                int pid, guess;
                                sscanf(buffer, "%c %d %d", &com, &pid, &guess);
                                if (guess == deck[12]) {
                                    // Correct : on attribue la victoire en réutilisant 'M'
                                    sprintf(reply, "M %d", pid);
                                    broadcastMessage(reply);
                                    fsmServer = 2; // Fin de partie
                                } else {
                                    // Incorrect : on passe au joueur suivant
                                    int next = (pid + 1) % 4;
                                    sprintf(reply, "M %d", next);
                                    broadcastMessage(reply);
                                }
                                break;
                            }
			...
			{
                            int pid, obj;
                            sscanf(buffer, "%c %d %d", &com, &pid, &obj);
                            // Envoyer à tous la valeur tableCartes[p][obj]
                            for (int p = 0; p < 4; p++) {
			...
			case 'S':
                        {
                            int pid, target, obj;
                            sscanf(buffer, "%c %d %d %d", &com, &pid, &target, &obj);
                            // Répondre uniquement au demandeur
                            sprintf(reply, "V %d %d %d", target, obj, tableCartes[target][obj]);
                            sendMessageToClient(tcpClients[pid].ipAddress,
                                                tcpClients[pid].port,
                                                reply);
                            // Tour suivant
                            int next = (pid + 1) % 4;
